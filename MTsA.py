...
def PLOT_button():
    ...
    if 'file_name' not in globals():
        tkM.showerror("","Please select a file!",icon = 'error')
    elif file_name == '':
        tkM.showerror("","Please select a file!",icon = 'error')
    else:
        entries_check = 0
        try:
            ...
            if time_col <= 0:
                entries_check = 1
                tkM.showerror("","Time column must be a positive number greater than zero! Please reinsert value",icon = 'error')
            data_col = int(entry_1[1].get())
            if data_col <= 0:
                entries_check = 1
                tkM.showerror("","Data column must be a positive number greater than zero! Please reinsert value",icon = 'error')
            Delta_t = float(entry_1[2].get())
            if Delta_t <= 0:
                entries_check = 1
                tkM.showerror("","Sampling time must be a positive number greater than zero! Please reinsert value",icon = 'error')
            ...
            tkM.showerror("","Not all variables inserted!",icon = 'error')
...
def OK_button():
    ...
    try:
        ...
        if year_in <= 0 or year_fin <= 0 or year_fin <= year_in:
            entries_check = 1
            tkM.showerror("","Invalid years! Please reinsert value",icon = 'error')
        ofac = int(entry_2[2].get())
        if ofac <= 0:
            entries_check = 1
            tkM.showerror("","Oversampling factor must be a positive number greater than zero! Please reinsert value",icon = 'error')
        ...
        if units_converter not in units_choices.values():
            entries_check = 1
            tkM.showerror("","Invalid time units! Please select one",icon = 'error')
        typeoffit = var_fit.get()
        if typeoffit not in fit_type:
            entries_check = 1
            tkM.showerror("","Invalid fit function! Please select one",icon = 'error')
        scale_min = int(entry_3[0].get())
        if scale_min < 3:
            entries_check = 1
            tkM.showerror("","Minimum scale for DFA should be at least 3! Please reinsert value",icon = 'error')
        scale_MFDFA = [int(i) for i in entry_3[1].get().split(',')]
        for i in scale_MFDFA:
            if i < 3:
                entries_check = 1
                tkM.showerror("","Scales for local Hurst exponent can not be less than 3! Please reinsert value",icon = 'error')
    except:
        entries_check = 1
        tkM.showerror("","Not all variables inserted!",icon = 'error')
    ...
    if entries_check == 0:
        ...
        ### replot time series in case plot button was not pressed
        file_plot(file_name,data_col,time_col,Delta_t,typeoffit)
        ...
        progressbar = ttk.Progressbar(top_bar,variable = progress_var,length = 350,maximum = 9,mode = 'determinate',style = "TProgressbar")
